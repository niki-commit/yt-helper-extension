...
export const isPlayerReady = (): boolean => {
  ...
  if (!ready) {
      console.debug('[PlayerCheck] video exists but metadata not ready. duration:', video.duration, 'readyState:', video.readyState);
  }
  return ready;
};

export const waitForPlayer = async (timeoutMs: number = 10000): Promise<boolean> => {
